...
cppFunction(
  ...
  , includes = '#include "geometries/coordinates/dimensions.hpp"'
  , code = '
    SEXP geometry_dimensions( SEXP x ) {
      return geometries::coordinates::geometry_dimensions( x );
    }
  '
)

## i.e., calculates the dimension for each list element.
## and returns a list with three elements, 
## $dimensions is a matrix giving the 
## - start index
## - end index
## - dimenson (number of values in a coordinate)
## - nesting (depth of element, e.g. list[[ mat ]] == depth 1)
## - RTYPE (13 = int, 14 = numeric)
...
## list represing single geometry of polygon
l <- list(
  matrix(1:20, ncol = 2 )
  , matrix(1:6, ncol = 2)
  , matrix(1:10, ncol = 2)
...
## 3 list elements, rows of the matrix
geometry_dimensions( l )

## list representing three geometries of line, line, polygon
l <- list(
  matrix(1:20, ncol = 2 )
  , matrix(1:6, ncol = 2)
  , list( matrix(1:10, ncol = 2) )
)
## 3 list elements, one is nested deeper
geometry_dimensions( l )
...
## still one list element, but nested a bit deeper
l <- list( l )
geometry_dimensions( l )
