...
cppFunction(
  ...
  , code = '
    SEXP my_shape( DataFrame df, IntegerVector id_cols, IntegerVector geometry_cols ) {
      return geometries::make_geometries( df, id_cols, geometry_cols );
    }
  '
)

my_shape( df, c(0,1), c(2,3) )
```


If you want to define a class for each geometry you can supply a "class" vector as the 4th argument
...
cppFunction(
  ...
  , includes = '#include "geometries/geometries.hpp"'
  , code = '
    SEXP my_shape( Rcpp::DataFrame df, Rcpp::IntegerVector id_cols, Rcpp::IntegerVector geometry_cols, Rcpp::StringVector class_attributes ) {
      return geometries::make_geometries( df, id_cols, geometry_cols, class_attributes );
    }
  '
)

my_shape( df, c(0,1), c(2,3), "my_polygon")

```

Notice here that each list element now has a `"my_polygon"` class.

And if you have `library(sf)` loaded, setting the class as `sfg` `POLYGON`, you should see each elemented printed in the usual `sf` way

```{r}

my_shape( df, c(0,1), c(2,3), c("XY", "POLYGON","sfg") )
...
my_shape( df, c(0,1,4,5), c(2,3), "my_new_shape")
